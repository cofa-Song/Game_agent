...
const submitAudit = async () => {
  ...
  if (modalAction.value === 'reject' && !inputReason.value.trim()) {
      alert(t('audits.reject_reason_empty'))
      return
  }
  ...
  if (index !== -1) {
    ...
    if (r) {
      if (modalAction.value === 'approve') {
         r.status = 'approved'
         r.reason = inputReason.value || t('audits.approve_default_remark')
...
const getStatusConfig = (status: string) => {
  switch(status) {
    case 'pending': return { label: t('audits.status_pending'), class: 'bg-amber-100 text-amber-700 border-amber-200', dot: 'bg-amber-500 animate-pulse' }
    case 'approved': return { label: t('audits.status_approved'), class: 'bg-emerald-100 text-emerald-700 border-emerald-200', dot: 'bg-emerald-500' }
    case 'rejected': return { label: t('audits.status_rejected'), class: 'bg-rose-100 text-rose-700 border-rose-200', dot: 'bg-rose-500' }
    default: return { label: t('withdrawal.unknown'), class: 'bg-slate-100 text-slate-700 border-slate-200', dot: 'bg-slate-400' }
  }
}

const getMethodConfig = (method: string) => {
  switch(method) {
    case 'bank': return { label: t('withdrawal.method_bank') }
    case 'usdt': return { label: t('withdrawal.method_usdt') }
    case 'offline': return { label: t('withdrawal.method_offline') }
    default: return { label: t('withdrawal.unknown') }
  }
}
</script>
...
<template>
  <div class="max-w-[1400px] mx-auto space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold tracking-tight text-slate-900">{{ t('audits.title') }}</h1>
        <p class="text-sm text-slate-500 mt-1">{{ t('audits.subtitle') }}</p>
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col min-h-[600px]">
      ...
      <div class="px-6 py-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/50 relative">
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          <!-- Status Filter -->
          <div class="flex overflow-x-auto no-scrollbar rounded-lg border border-slate-200 bg-slate-100 p-1">
            <button 
              v-for="status in [{v:'all', l:t('audits.status_all')}, {v:'pending', l:t('audits.status_pending')}, {v:'approved', l:t('audits.status_approved')}, {v:'rejected', l:t('audits.status_rejected')}]"
              :key="status.v"
              @click="filterStatus = status.v as any"
              class="px-3 sm:px-4 py-1.5 text-xs font-bold rounded-md transition-all shadow-sm whitespace-nowrap"
      ...
      <!-- Data Table (Desktop) -->
      <div class="flex-1 overflow-x-auto hidden lg:block">
        <table class="w-full text-sm text-left whitespace-nowrap">
          <thead class="text-xs text-slate-500 bg-slate-50/80 uppercase border-b border-slate-200">
            <tr>
              <th scope="col" class="px-6 py-4 font-bold">{{ t('audits.col_date_id') }}</th>
              <th scope="col" class="px-6 py-4 font-bold">{{ t('audits.col_agent') }}</th>
              <th scope="col" class="px-6 py-4 font-bold text-right">{{ t('audits.col_amount') }}</th>
              <th scope="col" class="px-6 py-4 font-bold">{{ t('audits.col_info') }}</th>
              <th scope="col" class="px-6 py-4 font-bold text-center">{{ t('audits.col_status') }}</th>
              <th scope="col" class="px-6 py-4 font-bold text-center sticky right-0 bg-slate-50 shadow-[-10px_0_15px_-3px_rgba(0,0,0,0.05)] border-l border-slate-100">{{ t('audits.col_actions') }}</th>
            </tr>
          </thead>
            ...
            <tr v-if="filteredRecords.length === 0">
              <td colspan="6" class="px-6 py-20 text-center text-slate-400">
                <div class="flex flex-col items-center justify-center gap-3">
                  ...
                  <p class="text-sm font-medium">{{ t('audits.empty_records') }}</p>
                </div>
              </td>
            </tr>
            <tr 
            ...
            >
              ...
              <td class="px-6 py-4 text-right">
                <div class="flex flex-col items-end">
                  ...
                  <span class="text-[10px] text-slate-400" :title="t('audits.frozen_balance')">
                    ({{ t('audits.frozen_balance') }}: ${{ formatCurrency(record.agent.frozenCommission) }})
                  </span>
                </div>
              </td>
      ...
      <!-- Mobile Card List -->
      <div class="lg:hidden flex-1 bg-slate-50/50 p-4 space-y-4 overflow-y-auto">
        <div v-if="filteredRecords.length === 0" class="py-20 text-center text-slate-400">
          <p class="text-sm">{{ t('audits.empty_records') }}</p>
        </div>
        <div 
          v-for="record in filteredRecords" 
          :key="record.id" 
          class="bg-white rounded-xl border border-slate-200 p-4 shadow-sm space-y-4"
        >
          <div class="flex justify-between items-start">
            <div class="flex flex-col gap-1">
               <span class="text-[10px] font-mono text-indigo-500 bg-indigo-50 px-1.5 py-0.5 rounded self-start">{{ record.id }}</span>
               <span class="text-xs text-slate-500">{{ record.date?.split('T')[0] }} {{ record.date?.split('T')[1].substring(0,5) }}</span>
            </div>
            <span 
          ...
          <div class="flex items-center gap-3 py-2 border-y border-slate-50">
             ...
             <div class="flex flex-col">
                <span class="font-bold text-slate-800 text-sm">{{ record.agent.name }}</span>
                <span class="text-[10px] text-slate-500 font-mono">UID: {{ record.agent.uid }}</span>
             </div>
             <div class="ml-auto text-right">
                <div class="text-rose-600 font-black text-lg">${{ formatCurrency(record.amount) }}</div>
                <div class="text-[10px] text-slate-400">{{ t('audits.frozen_balance') }}: ${{ formatCurrency(record.agent.frozenCommission) }}</div>
             </div>
          </div>

          <div class="space-y-2">
             <div class="flex flex-col gap-1">
                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{{ t('audits.col_info') }}</span>
                <div class="flex items-center gap-2">
                   <span class="text-xs font-bold text-slate-700">{{ getMethodConfig(record.method).label }}</span>
                   <span class="text-[11px] text-slate-500 font-mono">{{ record.accountInfo }}</span>
                </div>
             </div>
             <div v-if="record.reason" class="flex flex-col gap-1">
                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{{ t('audits.remark') }}</span>
                <p class="text-xs text-slate-600 italic bg-amber-50/50 p-2 rounded border border-amber-100/50">{{ record.reason }}</p>
             </div>
          </div>

          <div v-if="record.status === 'pending'" class="grid grid-cols-2 gap-3 pt-2">
            <button 
               @click="openModal(record, 'reject')"
               class="py-2 bg-white border border-rose-200 text-rose-600 text-xs font-bold rounded-lg hover:bg-rose-50"
            >
               {{ t('audits.btn_reject') }}
            </button>
            <button 
               @click="openModal(record, 'approve')"
               class="py-2 bg-emerald-500 text-white text-xs font-bold rounded-lg hover:bg-emerald-600 shadow-sm"
            >
               {{ t('audits.btn_approve') }}
            </button>
          </div>
      ...
      <div class="px-6 py-4 border-t border-slate-100 bg-slate-50/30 flex justify-between items-center text-xs text-slate-500 mt-auto">
        <span>{{ t('audits.pagination_showing', { start: 1, end: filteredRecords.length, total: filteredRecords.length }) }}</span>
      </div>
    </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md relative z-10 overflow-hidden" @click.stop>
          ...
          >
            <h3 class="text-lg font-bold flex items-center gap-2" :class="modalAction === 'approve' ? 'text-emerald-800' : 'text-rose-800'">
              ...
              {{ modalAction === 'approve' ? t('audits.modal_approve_title') : t('audits.modal_reject_title') }}
            </h3>
          ...
          <div class="p-6 space-y-6">
              ...
              <div class="pt-2 mt-2 border-t border-slate-200">
                <p class="text-xs text-slate-500 font-medium leading-relaxed">
                  <template v-if="modalAction === 'approve'">
                    {{ t('audits.modal_approve_note', { amount: '$' + formatCurrency(selectedRecord?.amount || 0) }) }}
                  </template>
                  <template v-else>
                    {{ t('audits.modal_reject_note', { amount: '$' + formatCurrency(selectedRecord?.amount || 0) }) }}
                  </template>
                </p>
              </div>
            </div>
            ...
            <div class="space-y-2">
              <label class="text-sm font-bold text-slate-700 flex justify-between">
                <span>{{ modalAction === 'approve' ? t('audits.modal_label_remark_approve') : t('audits.modal_label_remark_reject') }}</span>
                <span v-if="modalAction === 'reject'" class="text-rose-500 text-xs">{{ t('audits.modal_reason_required') }}</span>
              </label>
              <textarea 
                v-model="inputReason"
                rows="3"
                ...
                :placeholder="modalAction === 'approve' ? t('audits.modal_placeholder_approve') : t('audits.modal_placeholder_reject')"
              ></textarea>
            </div>
          </div>
          ...
          <div class="px-6 py-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
            ...
            >
              ...
              <span>{{ modalAction === 'approve' ? t('audits.btn_confirm_approve') : t('audits.btn_confirm_reject') }}</span>
            </button>
          </div>
        </div>
      </div>
